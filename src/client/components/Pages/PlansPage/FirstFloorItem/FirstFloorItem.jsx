...
export const FirstFloorItem = () => {
  const dispatch = useDispatch();
  const activeArea = useSelector(getActiveArea);

  useEffect(() => {
    dispatch(setTitle('Первый этаж'));
  ...
  const spTemperatureZalLeft = useSelector(getAIbyId('AI3000794')) || defaultAIData;
  const temperatureKitchenLeft = useSelector(getAIbyId('AI3000778')) || defaultAIData;
  const spTemperatureKitchenLeft = useSelector(getAIbyId('AI3000793')) || defaultAIData;
  ...
  const temperatureZalRight = useSelector(getAIbyId('AI3000172')) || defaultAIData;
  const spTemperatureZalRight = useSelector(getAIbyId('AI3000173')) || defaultAIData;
  ...
  const firstFloorHF_WC_Left_valve = useSelector(getAIbyId('AI3001178')) || defaultAIData;
  const isOnFirstFloorHF_WC_Left = firstFloorHF_WC_Left_valve.value > 5 && isOnPumpHF_L.value;

  const firstFloorHF_kitchen_Left_valve = useSelector(getAIbyId('AI3001176')) || defaultAIData;
  const isOnFirstFloorHF_kitchen_Left = firstFloorHF_kitchen_Left_valve.value > 5 && isOnPumpHF_L.value;

  const firstFloorHF_zal_Left_valve = useSelector(getAIbyId('AI3001180')) || defaultAIData;
  const isOnFirstFloorHF_zal_Left = firstFloorHF_zal_Left_valve.value > 5 && isOnPumpHF_L.value;

  const firstFloorHF_hall_Left_valve = useSelector(getAIbyId('AI3001177')) || defaultAIData;
  const isOnFirstFloorHF_hall_Left = firstFloorHF_hall_Left_valve.value > 5 && isOnPumpHF_L.value;

  const firstFloorHF_kitchen_Right_valve = useSelector(getAIbyId('AI3000182')) || defaultAIData;
  const isOnFirstFloorHF_kitchen_Right = firstFloorHF_kitchen_Right_valve.value > 5;

  const isOnFirstFloorHF_WC_Right = isOnFirstFloorHF_kitchen_Right;
  const isOnFirstFloorHF_hall_Right = isOnFirstFloorHF_WC_Right;

  const isOnPumpSO_R = useSelector(getBIbyId('BI3000247')) || defaultBOData;

  const firstFloorSO_zal_Right_valve = useSelector(getAIbyId('AI3000164')) || defaultAIData;
  const isOnFirstFloorSO_zal_Right = firstFloorSO_zal_Right_valve > 5 && isOnPumpSO_R.value;

  const firstFloorSO_kitchen_Right_valve = useSelector(getAIbyId('AI3000163')) || defaultAIData;
  const isOnFirstFloorSO_kitchen_Right = firstFloorSO_kitchen_Right_valve > 5 && isOnPumpSO_R.value;

  const isOnPumpSO_L = useSelector(getBIbyId('BI3000818')) || defaultBOData;

  const firstFloorSO_zal_Left_valve = useSelector(getAIbyId('AI3000788')) || defaultAIData;
  const isOnFirstFloorSO_zal_Left = firstFloorSO_zal_Left_valve > 5 && isOnPumpSO_L;

  const firstFloorSO_kitchen_Left_valve = useSelector(getAIbyId('AI3000786')) || defaultAIData;
  const isOnFirstFloorSO_kitchen_Left = firstFloorSO_kitchen_Left_valve > 5 && isOnPumpSO_L.value;

  const spTemperatureKitchenRight = useSelector(getAObyId('AO3000879')) || defaultAOData;

  const colorZalLeft = convertDiffTemperaturesToColor(
    temperatureZalLeft.value,
    spTemperatureZalLeft.value,
  );
  const colorZalRight = convertDiffTemperaturesToColor(
    temperatureZalRight.value,
    spTemperatureZalRight.value,
  );
  const colorKitchenLeft = convertDiffTemperaturesToColor(
    temperatureKitchenLeft.value,
    spTemperatureKitchenLeft.value,
  );
  const colorKitchenRight = convertDiffTemperaturesToColor(
    temperatureKitchenRight.value,
    spTemperatureKitchenRight.value,
  );
    ...
    <div className={styles.container}>
      <svg viewBox="20 0 360 450" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(95, 140)">
          <TemperatureTile temperature={valueToFixed(temperatureZalLeft.value)}/>
        </g>
        <g transform="translate(260, 140)">
          <TemperatureTile temperature={valueToFixed(temperatureZalRight.value)}/>
        </g>
        <g transform="translate(65, 310)">
          <TemperatureTile temperature={valueToFixed(temperatureKitchenLeft.value)}/>
        </g>
        <g transform="translate(290, 310)">
          <TemperatureTile temperature={valueToFixed(temperatureKitchenRight.value)}/>
        </g>
        <g transform="translate(320, 280)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFKitchenRight.value)}/>
        </g>
        <g transform="translate(95, 350)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFKitchenLeft.value)}/>
        </g>
        <g transform="translate(153, 287)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFHallLeft.value)}/>
        </g>
        <g transform="translate(151, 227)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFWC1Left.value)}/>
        </g>
        <g transform="translate(50, 120)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFZal_L_Left.value)}/>
        </g>
        <g transform="translate(151, 170)">
          <HFTemperatureTile temperature={valueToFixed(temperatureHFZal_R_Left.value)}/>
        </g>
